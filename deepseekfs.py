...
import shutil
import requests
import json # Import json for Deepseek
import uuid
import re
import io
import logging
...
from datetime import datetime # NEW import for SessionManager
...
import openai # Keep openai import for now, could be removed later if not used elsewhere
...
# Modified regeneration function
def regenerate_questions_with_feedback(current_questions: str, reference: str,
                                      base_prompt: str, feedback_handler: FeedbackHandler,
                                      thresholds: dict, user_feedback: str = None, max_retries=3):
    ...
    for attempt in range(max_retries):
        ...
        new_questions = get_gpt_response("Question Generation Expert", augmented_prompt)
...
# ---------------------------
# Function Definitions (rest of the functions remain mostly the same, only `generate_questions` and `main` are modified to use new feedback and session classes)
# ---------------------------
...
def get_gpt_response(system_prompt, user_prompt):
    """
    Gets a response from Deepseek (replacing GPT).
    """
    print("Sending prompt to Deepseek for generation...")
    deepseek_api_key = "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" # Replace with your actual Deepseek API key
    headers = {
...
def generate_questions(final_user_prompt_path, updated_final_user_prompt_path, retrieved_context, course_name, num_questions, academic_level, taxonomy, topics_list, major, evaluation_prompt_path, updated_evaluation_prompt_path, thresholds, feedback_handler, final_user_prompt): # Added feedback_handler and final_user_prompt
    """Generates questions based on retrieved context and user prompt, now with feedback handling."""
    ...
    initial_questions = get_gpt_response("You are a helpful assistant skilled at automatic question generation.", final_user_prompt_for_generation) # Use the filled prompt for generation
    ...
    regenerated_questions, feedback, scores = regenerate_questions_with_feedback(
        initial_questions, retrieved_context, final_user_prompt, feedback_handler, thresholds # Pass feedback_handler and original final_user_prompt
    )
...
def main():
    ...
    generated_questions, evaluation_feedback, scores  = generate_questions(
        ...
        topics_list, major, evaluation_prompt_path, updated_evaluation_prompt_path, thresholds, feedback_handler, final_user_prompt_content # Pass feedback handler and final user prompt content
    )
